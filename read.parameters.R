...
read.parameters <- function(params.file = "parameters.csv", estimate.saturation = FALSE, resource.file = "resources.csv"){
  try({
    ...
    parameter.list <- list(
      ...
      gamma.E                 = as.numeric(1/data.parameters["Days of latency (with virus but not contagious)",]),
      ...
      beta.2to3.saturated     = as.numeric(1/data.parameters["Days of severe case before critical without hospitalization",]),
      beta.2to3.not.saturated = as.numeric(1/data.parameters["Days of severe case before critical with hospitalization",]),
      beta.2toR.saturated     = as.numeric(1/data.parameters["Days of severe case before recovery without hospitalization",]),
      beta.2toR.not.saturated = as.numeric(1/data.parameters["Days of severe case before recovery with hospitalization",]),
      saturationI2            = Inf, 
      saturationI3            = Inf,
