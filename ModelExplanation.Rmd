...
params$q2 <- rep(1/2, 4)
model.7   <- run.model.continuous(params, model.6$state, init.time = 80, 
                                  end.time = 100)
```
...
model.3 <- run.model.continuous(params, model.2$state,  init.time = 30, end.time = 40)
model.4 <- run.model.continuous(params, model.3$state,  init.time = 40, end.time = 100)
...
plot <- geom_line.variable.from.list(model.list, plot, "I2")
plot <- geom_line.variable.from.list(model.list, plot, "I3")
plot + theme_bw()
```
